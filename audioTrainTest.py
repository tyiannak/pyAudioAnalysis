import sys, numpy, time, os, glob, mlpy, cPickle, shutil, alsaaudio, audioop, signal
...
def calibrationTraining(numOfBlocks, midTermBufferSizeSec, calibrationFileName, calibrationPath):

	MINSEGMENTSTORECORD = 5;

	# Open calibration list file
	try:
		calibrationClasses = [line.strip() for line in open(calibrationFileName)]	
	except IOError:
		print 'ERROR: Calibration classes file not found.'
		return

	# Create output folders:
	dirNames = []	
	dirNameNoise = calibrationPath + os.sep + "noise"
	if os.path.exists(calibrationPath):
		shutil.rmtree(calibrationPath)

	if os.path.exists(dirNameNoise):
		shutil.rmtree(dirNameNoise)
	os.makedirs(dirNameNoise)
	for c in calibrationClasses:
		dirNames.append(calibrationPath + os.sep + c)
		if os.path.exists(dirNames[-1]):
			shutil.rmtree(dirNames[-1])
		os.makedirs(dirNames[-1])
	
	# Initialize
	isOnset = False
	countOnSets = 0
	countOnSetSegments = 0
	if len(calibrationClasses)>countOnSets:
		curClass = calibrationClasses[countOnSets].upper()
	else:
		curClass = "UNKNOWN"
	...
	while count <= numOfBlocks:
		...
		if len(midTermBuffer) == midTermBufferSize:
			count += 1
			# stFeatures = aF.stFeatureExtraction(midTermBuffer, Fs, 400, 200)									
			signal = numpy.double(midTermBuffer)
			signal = signal / (2.0**15)
			DC     = signal.mean()	
			signal = (signal - DC) 

			curEnergy = aF.stEnergy(signal)
			if count==1:
				referenceEnergy = curEnergy
			else:
				Ratio = curEnergy / referenceEnergy
				Threshold = 5.0

				if (Ratio > Threshold):
					if not isOnset: # START recording current buffer
						isOnset = True
					print "%8.2f" % Ratio + " (" + "%10.8f" % curEnergy  + " / " + "%10.8f" % referenceEnergy + ") - - - - Writing segment " + str(countOnSetSegments) + " - - - - class name : " + curClass
					currentOnsetBufferNumpy = numpy.int16(midTermBuffer)

					if len(dirNames) > countOnSets:						
						curFileName = dirNames[countOnSets] + os.sep + str(countOnSets).zfill(3) + curClass + str(countOnSetSegments).zfill(3) + ".wav"
						wavfile.write(curFileName, Fs, currentOnsetBufferNumpy)
					
					countOnSetSegments += 1
				else:
					# update reference energy:
					# W = 0.8
					# referenceEnergy = W * referenceEnergy + (1.0 - W) * curEnergy
					if isOnset:	# STOP recording current buffer (offset)						
						isOnset = False						
						if countOnSetSegments >= MINSEGMENTSTORECORD:
							countOnSets += 1
							countOnSetSegments = 0
							if len(calibrationClasses)>countOnSets:
								curClass = calibrationClasses[countOnSets].upper()
								...
								curClass = "UNKNOWN"
					else:	# write NOISE (if the current block is not a "limit" block
						currentOnsetBufferNumpy = numpy.int16(midTermBuffer)
						curFileName = dirNameNoise + os.sep + "noise" + str(count).zfill(4) + ".wav"
						wavfile.write(curFileName, Fs, currentOnsetBufferNumpy)

					print "%8.2f" % Ratio + " (" + "%10.8f" % curEnergy  + " / " + "%10.8f" % referenceEnergy + ")" + " - - - - Waiting for class " + curClass
			allData = allData + midTermBuffer
			midTermBuffer = []
			
	allDataArray = numpy.int16(allData)
	dirNames.append(dirNameNoise)
	calibrationClasses.append('noise')

	outputWavFile = calibrationPath + ".wav"
	wavfile.write(outputWavFile, Fs, allDataArray)

	return (calibrationClasses, dirNames)


def calibrationTraining2(numOfBlocks, midTermBufferSizeSec, calibrationFileName, calibrationPath):

	REQUIRED_SAMPLES_PER_CLASS = numOfBlocks;
	...
	while ic < (len(calibrationClasses)):
		print chr(27) + "[2J"
		c = calibrationClasses[ic]
		d = dirNames[ic]
		midTermBuffer = []
		curWindow = []
		count = 0
		...
		raw_input("Press ENTER to start recording class " + c.upper() + " " + str(ic+1) + " (out of " + str(len(calibrationClasses)) + " total classes)")
		time.sleep(1.5)

		# Initialize
		inp = alsaaudio.PCM(alsaaudio.PCM_CAPTURE,alsaaudio.PCM_NONBLOCK)
		# Set attributes: Mono, 8000 Hz, 16 bit little endian samples
		inp.setchannels(1); inp.setrate(Fs); inp.setformat(alsaaudio.PCM_FORMAT_S16_LE); inp.setperiodsize(512)

		while count < REQUIRED_SAMPLES_PER_CLASS:
			# Read data from device
			l,data = inp.read()
		    	if l:
			    	# Return the maximum of the absolute value of all samples in a fragment.
			...
			if len(midTermBuffer) == midTermBufferSize:
				count += 1
				midTermBufferNumpy = numpy.int16(midTermBuffer)
				curFileName = d + os.sep + str(count).zfill(3) + c + str(countOnSetSegments).zfill(3) + ".wav"
				wavfile.write(curFileName, Fs, midTermBufferNumpy)
				countOnSetSegments += 1
				midTermBuffer = []
				print "Recording class " + c.upper() + " - " + str(midTermBufferSizeSec * (REQUIRED_SAMPLES_PER_CLASS - countOnSetSegments)).zfill(3) + " seconds remaining"


		print "Was the recording of the current class (" + c.upper() + ") successful? [y/n]:" 
		os.system("stty cbreak -echo")
		...
		if ANS.lower()=='y':
			...
			print "Recording of " + c.upper() + " is going to restart."
			if os.path.exists(d):
				shutil.rmtree(d)
			os.makedirs(d)
	return (calibrationClasses, dirNames)
