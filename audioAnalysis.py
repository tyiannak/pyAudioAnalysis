...
def dirMp3toWavWrapper(directory, samplerate, channels):
    ...
    audioBasicIO.convertDirMP3ToWav(
...
def featureExtractionFileWrapper(wavFileName, outFile, mtWin, mtStep, stWin, stStep):
    ...
    aF.mtFeatureExtractionToFile(
        wavFileName, mtWin, mtStep, stWin, stStep, outFile, True, True, True)
...
def featureExtractionDirWrapper(directory, mtWin, mtStep, stWin, stStep):
    ...
    aF.mtFeatureExtractionToFileDir(
        directory, mtWin, mtStep, stWin, stStep, True, True, True)
...
def silenceRemovalWrapper(inputFile, smoothingWindow, weight):
    ...
    segmentLimits = aS.silenceRemoval(
        x, Fs, 0.05, 0.05, smoothingWindow, weight, True)    # get onsets
...
def thumbnailWrapper(inputFile, thumbnailWrapperSize):
    ...
    [A1, A2, B1, B2, Smatrix] = aS.musicThumbnailing(
        x, Fs, stWindow, stStep, thumbnailWrapperSize)    # find thumbnailWrapper endpoints
    ...
    e1 = matplotlib.patches.Ellipse(
        (Ycenter, Xcenter), thumbnailWrapperSize * 1.4, 3, angle=45, linewidth=3, fill=False)
    ax.add_patch(e1)
...
def parse_arguments():
    ...
    dirMp3Wav = tasks.add_parser(
        "dirMp3toWav", help="Convert .mp3 files in a directory to .wav format")
    ...
    featExtDir.add_argument("-mw", "--mtwin", type=float,
                            required=True, help="Mid-term window size")
    featExtDir.add_argument("-ms", "--mtstep", type=float,
                            required=True, help="Mid-term window step")
    featExtDir.add_argument("-sw", "--stwin", type=float,
                            default=0.050, help="Short-term window size")
    featExtDir.add_argument("-ss", "--ststep", type=float,
                            default=0.050, help="Short-term window step")
    ...
    chroma.add_argument("-i", "--input", required=True,
                        help="Input audio file")
    ...
    trainClass.add_argument("--method", choices=["svm", "svm_rbf", "knn", "randomforest",
                                                 "gradientboosting", "extratrees"], required=True, help="Classifier type")
    ...
    trainReg.add_argument(
        "--method", choices=["svm", "randomforest", "svm_rbf"], required=True, help="Classifier type")
    ...
    classFile.add_argument("-i", "--input", required=True,
                           help="Input audio file")
    classFile.add_argument("--model", choices=["svm", "svm_rbf", "knn", "randomforest", "gradientboosting", "extratrees"],
                           required=True, help="Classifier type (svm or knn or randomforest or gradientboosting or extratrees)")
    ...
    trainHMM.add_argument("-mw", "--mtwin", type=float,
                          required=True, help="Mid-term window size")
    trainHMM.add_argument("-ms", "--mtstep", type=float,
                          required=True, help="Mid-term window step")

    trainHMMDir = tasks.add_parser(
        "trainHMMsegmenter_fromdir", help="Train an HMM from file + annotation data stored in a directory (batch)")
    trainHMMDir.add_argument(
        "-i", "--input", required=True, help="Input audio folder")
    trainHMMDir.add_argument(
        "-o", "--output", required=True, help="HMM model name (path)")
    trainHMMDir.add_argument("-mw", "--mtwin", type=float,
                             required=True, help="Mid-term window size")
    trainHMMDir.add_argument(
        "-ms", "--mtstep", type=float, required=True, help="Mid-term window step")

    segmentClassifyFile = tasks.add_parser(
        "segmentClassifyFile", help="Segmentation - classification of a WAV file given a trained SVM or kNN")
    segmentClassifyFile.add_argument(
        "-i", "--input", required=True, help="Input audio file")
    segmentClassifyFile.add_argument(
        "--model", choices=["svm", "svm_rbf", "knn", "randomforest", "gradientboosting", "extratrees"], required=True, help="Model type")
    segmentClassifyFile.add_argument(
        "--modelName", required=True, help="Model path")

    segmentClassifyFileHMM = tasks.add_parser(
        "segmentClassifyFileHMM", help="Segmentation - classification of a WAV file given a trained HMM")
    ...
    segmentationEvaluation = tasks.add_parser(
        "segmentationEvaluation", help="Segmentation - classification evaluation for a list of WAV files and CSV ground-truth stored in a folder")
    ...
    regFile.add_argument(
        "--model", choices=["svm", "svm_rbf", "randomforest"], required=True, help="Regression type")
    ...
    classFolder.add_argument(
        "-i", "--input", required=True, help="Input folder")
    classFolder.add_argument("--model", choices=["svm", "svm_rbf", "knn", "randomforest",
                                                 "gradientboosting", "extratrees"], required=True, help="Classifier type")
    classFolder.add_argument(
    ...
    silrem.add_argument("-i", "--input", required=True,
                        help="input audio file")
    silrem.add_argument("-s", "--smoothing", type=float,
                        default=1.0, help="smoothing window size in seconds.")
    silrem.add_argument("-w", "--weight", type=float,
                        default=0.5, help="weight factor in (0, 1)")

    spkrDir = tasks.add_parser("speakerDiarization")
    spkrDir.add_argument("-i", "--input", required=True,
                         help="Input audio file")
    spkrDir.add_argument("-n", "--num", type=int,
                         required=True, help="Number of speakers")
    ...
    thumb.add_argument("-s", "--size",  default=10.0,
                       type=float, help="thumbnailWrapper size in seconds.")
...
if __name__ == "__main__":
    ...
    # Train a regression model from data (organized in a single folder, while
    # ground-truth is provided in a CSV)
    ...
    # Train an hmm segmenter-classifier from a list of WAVs and annotations
    # stored in a folder
    elif args.task == "trainHMMsegmenter_fromdir":
        trainHMMsegmenter_fromdir(
            args.input, args.output, args.mtwin, args.mtstep)
    # Apply a classifier (svm or knn or randomforest or gradientboosting or
    # extratrees) for segmentation-classificaiton to a WAV file
    ...
    # Evaluate segmentation-classification for a list of WAV files (and ground
    # truth CSVs) stored in a folder
    ...
    # Classify every WAV file in a given path
        ...
        classifyFolderWrapper(args.input, args.model,
                              args.classifier, args.details)
    ...
    # Detect non-silent segments in a WAV file and output to seperate WAV files
    ...
    # Evaluate speaker diarization given a folder that contains WAV files and
    # .segment (Groundtruth files)
